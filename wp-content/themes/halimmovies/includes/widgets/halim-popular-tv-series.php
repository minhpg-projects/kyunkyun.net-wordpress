...
class halim_popular_tv_series_Widget extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'halim_popular_tv_series-widget',
			__( 'HaLim TOP TV Series', 'halimthemes' ),
			array(
				'classname'   => 'halim_popular_tv_series-widget',
				'description' => __( 'Display popular post by tv series', 'halimthemes' )
			)
		);
	}
	...
	{
		$defaults = array(
			'title' 		=> __('TOP Series', 'halimthemes'),
			'postnum' 		=> 6,
		);
...
function halim_popular_tv_series_Widgets(){
	register_widget('halim_popular_tv_series_Widget');
}
add_action('widgets_init', 'halim_popular_tv_series_Widgets');