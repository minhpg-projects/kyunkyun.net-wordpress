...
add_action( 'widgets_init', function () {
	register_widget( 'HaLim_Theater_Movie_Widget' );
} );

class HaLim_Theater_Movie_Widget extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'halim-theater-movie-widget',
			__( 'HaLim Theater Movie Widget', 'halimthemes' ),
			array(
				'classname'   => 'halim-theater-movie-widget',
				'description' => __( 'Display posts by Theater Movies', 'halimthemes' )
			)
		);
	}
	...
	public function widget( $args, $instance )
	{
		...
		<section id="halim-theater_movie-widget-<?php echo $widget_id; ?>" class="halim-theater_movie-widget-<?php echo $widget_id; ?>" style="position: relative;">
					...
					<div class="section-title">
						...
						<ul class="halim-popular-tab" role="tablist">
							...
							<li role="presentation"<?php echo 'latest' == $type ? ' class="active"' : ''; ?>>
								<a onclick="HaLim.GetPostByWidgetType('theater_movie', '<?php echo $layout; ?>', <?php echo $postnum; ?>, 'latest', '<?php echo $widget_id; ?>');" role="tab" data-toggle="tab"><?php _e('Mới nhất', 'halimthemes') ?></a>
							</li>
							<li role="presentation"<?php echo 'lastupdate' == $type ? ' class="active"' : ''; ?>>
								<a onclick="HaLim.GetPostByWidgetType('theater_movie', '<?php echo $layout; ?>', <?php echo $postnum; ?>, 'lastupdate', '<?php echo $widget_id; ?>');" role="tab" data-toggle="tab"><?php _e('Mới cập nhật', 'halimthemes') ?></a>
							</li>
							<li role="presentation"<?php echo 'mostview' == $type ? ' class="active"' : ''; ?>>
								<a onclick="HaLim.GetPostByWidgetType('theater_movie', '<?php echo $layout; ?>', <?php echo $postnum; ?>, 'mostview', '<?php echo $widget_id; ?>');" role="tab" data-toggle="tab"><?php _e('Nhiều lượt xem', 'halimthemes') ?></a>
							</li>
			...
			<div class="halim_box" id="ajax-theater_movie-widget-<?php echo $widget_id; ?>">
	...
	public function form( $instance ) {
		...
		<div class="hl_slider_form">
			...
			<p>
				...
				<br>
				<?php
				  $f = array( 'latest' => __('Mới nhất', 'halimthemes'), 'lastupdate' => __('Mới cập nhật', 'halimthemes'), 'mostview' => __('Most watched', 'halimthemes'));
					foreach ($f as $x => $n ) { ?>
